...
* **Acceptance Criteria (Gherkin):**
    ...
    * **Scenario: External Threat Prevention**
        * **Given** an unauthorized attempt to connect via external tools (e.g., DataGrip).
...
**Goal:** Implement strict validations and access control.
...
* **Acceptance Criteria (Gherkin):**
    * **Scenario: Strict Password Enforcement**
        ...
        * **When** the user provides a 12-character password and a @gmail.com email.
        * **Then** the account is successfully created.
    * **Scenario: Short Password Rejection**
        * **Given** a user entering an 11-character password.
        * **When** they submit.
        * **Then** the system displays: "Error: Security standard requires 12 characters minimum."
...
* **Acceptance Criteria (Gherkin):**
    * **Scenario: Instant Price Sync via QR**
        * **Given** a customer scanning the store's QR code.
        * **When** the staff updates "Drinks" prices.
        * **Then** the customer's view must update in less than 2 seconds.
...
* **Acceptance Criteria (Gherkin):**
    * **Scenario: Category Revenue Report**
        * **Given** the sales database with active JOINs.
        * **When** I request the total revenue for "Desserts".
        * **Then** the system calculates the sum using SQL SUM functions.
...
**So que** the shop can recover from technical failures in minutes.

* **Acceptance Criteria (Gherkin):**
    * **Scenario: Full System Restoration**
        * **Given** a database failure.
        * **When** the DBA initiates the SQL restore process.
        * **Then** all 5 categories and 50 products must be restored with 100% accuracy.
...
2. **NFR-02: User Feedback:** Immediate UI feedback for authentication errors.

...
* **Technical Protocol:**
  ...
  * El sistema debe escribir una fila en la tabla `staff_activity_log` documentando el nombre específico de la operadora (Dulce o Wendy), la marca de tiempo de ejecución y los parámetros delta de datos.
...
The architectural design utilizes specific programming structures, frameworks, and component versions optimized for the Mielecita platform deployment:

| System Layer | Technology / Language | Version / Context | Framework & Developer Tools | Operational Responsibility |
| :--- | :--- | :--- | :--- | :--- |
| **Frontend Environment** | Node.js | v20.11.0 (LTS) | npm Package Manager | Scaffolds development runtimes and manages local project dependencies. |
| **Frontend Core** | JavaScript / TypeScript | ES2022 | Vite Dev Server v5.2 | Compiles responsive structures and handles runtime asynchronous type safety. |
| **Presentation Engine** | React.js | v18.3.1 | Lovable Component Compiler | Renders dynamic marketplace card modules and retains client-side interface state. |
| **User Interface Styles** | CSS3 Grid Layouts | Utility-First | Tailwind CSS Framework v3.4 | Provides responsive grid layouts, styling variables, and grayscale dynamic classes. |
| **Data Query Cache** | TanStack Query | v5.28 | React Query Ecosystem | Handles synchronization, client state polling, and server payload response caching. |
| **Application Router** | Client Navigation | Route Parameters | React Router DOM v6.22 | Decouples frontend views and handles private links (`/personal/:nombre...`). |
| **Cloud Middleware** | Supabase BaaS | REST API Endpoint | Supabase JavaScript SDK v2.43 | Acts as the backend security perimeter gate and maps public database connections. |
| **Realtime Channel** | WebSockets | Push Stream | Supabase Realtime Protocol | Broadcasts data changes between backend inventory rows and customer view instances. |
| **Database Core** | PostgreSQL | v15.6 / v16.1 | Cloud Relational Engine | Manages atomicity, ensures 3NF data relations, and handles core data assets storage. |
| **Server-Side Engine** | PL/pgSQL | Procedural Extension | PostgreSQL Native Trigger | Runs security-defined triggers to automatically write audit rows tracking operator actions. |

---

## 5. Data Requirements & Normalization Matrix
